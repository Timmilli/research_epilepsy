...
def preprocess_fsl(params):
    ...
    t1_files = sorted(glob.glob(os.path.join(
        data_dir, "*/t1.nii.gz"), recursive=True))
    fl_files = sorted(glob.glob(os.path.join(
        data_dir, "*/flair.nii.gz"), recursive=True))
    fl_roi_files = sorted(glob.glob(os.path.join(
        data_dir, "*/flair_roi.nii.gz"), recursive=True))

    for i in range(len(fl_files)):
        if i >= len(fl_roi_files):
            fl_roi_files.append("")
        else:
            fl_id = fl_files[i][13:21]
            roi_id = fl_roi_files[i][13:21]
            if fl_id != roi_id:
                fl_roi_files.insert(i, "")
    ...
    for i in range(sub_count):
        ...
        strcmd = 'convert_xfm -omat {} -inverse {}'.format(
            mat_std2fov, mat_fov2std)
        os.system(strcmd)

        mat_ori2fov = os.path.join(sub_save_dir, 'ori2fov.mat')
        strcmd = 'convert_xfm -omat {} -concat {} {}'.format(
            mat_ori2fov, mat_std2fov, mat_ori2std)
        os.system(strcmd)

        mat_ori2mni = os.path.join(sub_save_dir, 'ori2mni.mat')
        strcmd = 'convert_xfm -omat {} -concat {} {}'.format(
            mat_ori2mni, mat_fov2mni, mat_ori2fov)
        os.system(strcmd)

        strcmd = 'flirt -in {} -ref {} -out {} -init {} -interp trilinear -applyxfm'.format(
            sub_t1_path, mni152_path, sub_t1_reg, mat_ori2mni)
        os.system(strcmd)

        if sub_fl_roi_path != "":
            # === Apply ori2mni transform to t1_roi.nii.gz
            sub_fl_roi_reg = os.path.join(sub_save_dir, 'fl_roi_reg.nii.gz')
            strcmd = 'flirt -in {} -ref {} -out {} -init {} -interp nearestneighbour -applyxfm'.format(
                sub_fl_roi_path, mni152_path, sub_fl_roi_reg, mat_ori2mni)
            os.system(strcmd)
        ...
        strcmd = 'flirt -in {} -ref {} -out {} -init {} -interp trilinear -applyxfm'.format(
            sub_fl_reg0, sub_t1_reg, sub_fl_reg, mat_ori2mni)
        os.system(strcmd)

        # ========================================================
        gt_files = sorted(glob.glob(os.path.join(
            data_dir, name, "gt.nii.gz"), recursive=True))
        if len(gt_files) == 0:
            continue
        ...
        strcmd = 'flirt -in {} -ref {} -out {} -init {} -interp nearestneighbour -applyxfm'.format(
            sub_gt_path, sub_t1_path, sub_gt_reg0, mat_fl2t1)
        os.system(strcmd)

        sub_gt_reg = os.path.join(sub_save_dir, 'gt_reg.nii.gz')
        strcmd = 'flirt -in {} -ref {} -out {} -init {} -interp nearestneighbour -applyxfm'.format(
            sub_gt_reg0, sub_t1_reg, sub_gt_reg, mat_ori2mni)
        os.system(strcmd)
